...
proto(InsertQueue, {
    itemsToBatches: function(items, batchSize) {
    ...
    schedule: function(op, items, batchSize, batchTm, targetItem) {
        this._schedule(items, targetItem, batchSize, batchTm, op, this._exec);
    },

    scheduleFnExec: function(items, batchSize, batchTm, fn) {
        var batchTm = batchTm || C.INSERT_BATCH_DELAY;
        var itemBatches = this.itemsToBatches(items, batchSize);

        for(var i = 0; i < itemBatches.length; i++) {
            (function(ib, i) {
                setTimeout(fn(ib), batchTm * i);
            })(itemBatches[i], i);
        }
    },

    _schedule: function(items, targetItem, batchSize, batchTm, op, opFn) {
        var me = this;
        ...
        this.scheduleFnExec(items, batchSize, batchTm, function(items) {
            schedule.call(me, items);
        });
    },
