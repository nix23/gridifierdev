...
proto(DiscretizerCore, {
    ...
    discretizeOnDefAppend: function(xStep, yStep) {
        var cc = {
            vg: function(currY, currX, xStep, yStep) {
                return {
                    x1: currX - xStep + 1, x2: currX,
                    y1: currY + 1, y2: currY + yStep
                };
            },
            hg: function(currX, currY, yStep, xStep) {
                return {
        ...
        if(settings.eq("grid", "vertical"))
            return this._onDefAppend(xStep, yStep, grid.y2(), grid.y2(), cc.vg);
        else
            return this._rev(this._onDefAppend(yStep, xStep, grid.x2(), grid.x2(), cc.hg));
    },
    ...
    discretizeOnRevAppend: function(xStep, yStep) {
        ...
        if(settings.eq("grid", "vertical"))
            return this._onRevAppend(xStep, yStep, grid.x2(), grid.y2(), cc.vg);
        else
            return this._rev(this._onRevAppend(yStep, xStep, grid.y2(), grid.x2(), cc.hg));
    },
    ...
    normalizeCnXCoords: function(item, cn) {
        var params = [item, cn, "Width", "x2", "x1", grid.x2()];
        params.push(settings.eq("grid", "vertical") ? true : false);

        return this._normalizeCnXYCoords.apply(this, params);
    },

    normalizeCnYCoords: function(item, cn) {
        var params = [item, cn, "Height", "y2", "y1", grid.y2()];
        params.push(settings.eq("grid", "vertical") ? false : true);
