...
proto(CrsShifter, {
    ...
    shiftAll: function() {
        var shifts = [
            [CRS.LEFT.TOP, "LeftTop"],
            [CRS.LEFT.BOTTOM, "LeftBottom"],
            [CRS.BOTTOM.RIGHT, "BottomRight"],
            [CRS.BOTTOM.LEFT, "TopLeft"],
            [CRS.TOP.LEFT, "TopLeft"],
            [CRS.TOP.RIGHT, "BottomRight"],
            [CRS.RIGHT.BOTTOM, "RightBottom"],
            [CRS.RIGHT.TOP, "RightTop"]
        ];
        ...
        for(var i = 0; i < this._crs.length; i++) {
            ...
            for(var j = 0; j < shifts.length; j++) {
                if(connectors.eq(this._crs[i], shifts[j][0])) {
                    this["_shift" + shifts[j][1]](this._crs[i]);
                    break;
                }
            }
        }
